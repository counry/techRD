...
void compress_Brotli(std::string file, std::string str)
{
    ...
    uint8_t *regen_buffer = NULL;
    uint8_t *src_str = NULL;
    ...
    src_str = (uint8_t *)malloc(src_size);
    if (src_str == NULL) {
        std::cout << "Failed to allocate memory for *src_str." << std::endl;
        goto _ERROR_RETURN;
    }
    memcpy(src_str, str.c_str(), src_size);
    
    //compression
    compressed_data = (uint8_t *)malloc(max_dst_size);
    if (compressed_data == NULL) {
        std::cout << "Failed to allocate memory for *compressed_data." << std::endl;
        goto _ERROR_RETURN;
    }
    memset(compressed_data, 0x0, sizeof(max_dst_size));
    compressed_data_ret = BrotliEncoderCompress(BROTLI_DEFAULT_QUALITY, BROTLI_DEFAULT_WINDOW, BROTLI_DEFAULT_MODE, src_size, src_str, &max_dst_size, compressed_data);
    ...
    if (compressed_data_ret == BROTLI_FALSE) {
 	...
 	{
    std::string data((char *)compressed_data, max_dst_size);
    ...
    //Decompression
    regen_buffer = (uint8_t *)malloc(src_size);
    if (regen_buffer == NULL) {
        std::cout << "Failed to allocate memory for *regen_buffer." << std::endl;
        goto _ERROR_RETURN;
    }
